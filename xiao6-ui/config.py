...
# Policy Engine 未显式注册工具的默认权限（auto/confirm/session/never）；默认 ask（每次手动确认）
...
def check_env_conflict():
    ...
    import os as _os
    env_key = _os.environ.get("AGNES_API_KEY", "")
    env_base = _os.environ.get("AGNES_BASE_URL", "")
    env_model = _os.environ.get("AGNES_MODEL", "")
    ...
    try:
        ...
        with open(os.path.join(_here, ".env"), "r", encoding="utf-8") as _f:
            for _line in _f:
                _line = _line.strip()
    ...
    if env_key and dotenv_key and env_key != dotenv_key:
        conflicts.append(f"AGNES_API_KEY: ENV(len={len(env_key)}) != .env(len={len(dotenv_key)})")
    if env_base and dotenv_base and env_base != dotenv_base:
        conflicts.append(f"AGNES_BASE_URL: ENV={env_base} != .env={dotenv_base}")
    if env_model and dotenv_model and env_model != dotenv_model:
        conflicts.append(f"AGNES_MODEL: ENV={env_model} != .env={dotenv_model}")

    if conflicts:
        print("[CONFIG_CONFLICT] 检测到环境变量与 .env 配置不一致:")
        for _c in conflicts:
            print(f"  - {_c}")
        print("  建议：启动前 unset AGNES_API_KEY 以确保使用 .env 中的 Key")
    else:
        print("[CONFIG_OK] 环境变量与 .env 配置一致（或未设置环境变量）")

    # 输出诊断摘要（脱敏）
    key_src = "ENV" if env_key else (".env" if dotenv_key else "MISSING")
    key_len = len(env_key) if env_key else (len(dotenv_key) if dotenv_key else 0)
    print(f"[DIAGNOSTIC] Provider=agnes Base={env_base or dotenv_base or 'DEFAULT'} Key=PRESENT(len={key_len}) Model={env_model or dotenv_model or 'DEFAULT'}")
...
def reload():
    ...
    THEME = os.environ.get("ZHUANGZHOU_THEME", "light")
