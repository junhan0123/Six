...
def _product_catalog() -> List[Capability]:
    return [
        ...
        Capability(
            ...
            data_source="SQLite zhuangzhou.db (memory_summary/learnings/memories)",
            keywords=["记忆", "记得", "之前", "上次", "历史", "总结过去", "我做过",
                       "开发内容", "记一下", "回忆"],
        ),
        Capability(
            id="knowledge", name="知识库", group=GROUP_KNOWLEDGE, icon="📚",
            description="本地知识文档检索与解析，基于 G:/ZhuangZhou/knowledge/*.md。",
            risk=Risk.LOW, permission=Permission.AUTO,
            input="查询", output="相关文档片段",
            entry="knowledge.search / knowledge_runtime.engine",
            data_source="纯文件库 G:/ZhuangZhou/knowledge/*.md",
        ...
        Capability(
            ...
            data_source="SQLite zhuangzhou.db (goals/tasks)",
        ...
        Capability(
            ...
            data_source="SQLite zhuangzhou.db (user_model 单行 JSON)",
