#!/usr/bin/env python3
"""庄周 · 笔记 / 画像 / 提醒 / 每日笔记（Obsidian 风格，SQLite + Markdown 混合）"""
...
def extract_daily_note():
    ...
    try:
        ...
        if len(rows) < 4:
            return  # 对话太少，不值得抽
        convo = "\n".join((("用户" if r == "user" else "庄周") + "：" + c) for r, c in rows[-24:])
        prompt = (
            "你是庄周的记忆整理模块。下面是一段今天的对话，请整理成一份结构化「每日笔记」"
...
def extract_profile():
    ...
    try:
        ...
        prompt = (
            "你是庄周的画像抽取模块。下面是一段用户近期的发言。请从中提炼出"
...
def extract_persons():
    ...
    try:
        ...
        recent = "\n".join(r[0] for r in rows)
        prompt = (
            "你是庄周的人物抽取模块。下面是一段用户近期发言。请从中识别被提及的真实人物"
