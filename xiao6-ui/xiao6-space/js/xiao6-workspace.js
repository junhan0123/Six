/* ═════════════════════════════════════════════════════════════════
   SIX · Advanced Workspace — app logic (xiao6-workspace.js)
...
(function () {
  ...
  var autoSpeak = (function () { try { return localStorage.getItem('xiao6_autoSpeak') !== '0'; } catch (e) { return true; } })();
  var sessionId = null;
  try { sessionId = localStorage.getItem('xiao6_sid'); } catch (e) {}
  if (!sessionId) { sessionId = 'xiao6-' + Date.now(); try { localStorage.setItem('xiao6_sid', sessionId); } catch (e) {} }
  ...
  function setState(st, opts) {
    ...
    // context state dot
    var cs = $('ctxStateDot'); if (cs) { cs.className = 'xiao6-statedot ' + ((low === 'thinking' || low === 'executing' || low === 'listening') ? 'ongoing' : (low === 'error' ? 'error' : (low === 'idle' || low === 'offline' ? 'done' : 'done'))); }
  ...
  function addNode(kind, meta) {
    ...
    var av = el('div', 'xiao6-avatar', kind === 'user' ? '你' : (kind === 'assistant' ? '小' : (kind === 'tool' ? '⚙' : (kind === 'approval' ? '!' : (kind === 'result' ? '✓' : '·')))));
  ...
  function sendChat(text, opts) {
    ...
    fetch('/api/chat', { method: 'POST', credentials: 'same-origin', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
      ...
      .catch(function (err) { busy = false; stream.finalize(); an.node.classList.remove('streaming'); an.bub.appendChild(el('div', 'xiao6-bubble-body')).innerHTML = '<span style="color:var(--xiao6-danger)">请求失败 · 请检查核心服务</span>'; setState('ERROR'); setTimeout(fetchSnapshot, 600); });
    ...
    function onTool(phase, tool, arg, ok) {
      if (phase === 'start') {
        ...
        qsa('.xiao6-node.tool').forEach(function (n) { if (n.dataset.toolnode) n.querySelector('.xiao6-tool-summary').innerHTML = '工具 <b>' + esc(tool || '') + '</b> ' + (ok === false ? '失败' : '完成'); });
        renderAgent();
      }
      renderAgent();
    }
  ...
  function renderHome() {
    ...
    // runtime line
    var rl = $('homeRuntime'); if (rl) rl.innerHTML = '<i class="xiao6-dot"></i><span>' + (CORE_TEXT[String(snap.agent.state || 'IDLE').toUpperCase()] || '在线待命') + '</span>';
    ...
    var rc = $('homeRecent'); if (rc) {
      getJSON('/api/chat/history?limit=6').then(function (d) {
        ...
        msgs = msgs.slice(-5);
        rc.innerHTML = msgs.length ? msgs.map(function (m) { return '<div class="xiao6-home-item"><span class="dot"></span><span>' + esc(String(m.content || '').slice(0, 40)) + '</span></div>'; }).join('') : '<span class="xiao6-empty">暂无对话</span>';
      });
    }
    // active tasks
    var ht = $('homeTasks'); if (ht) {
      ...
      ht.innerHTML = open.length ? open.map(function (t) { return '<div class="xiao6-home-item"><span class="dot"></span><span>' + esc(t.title || '任务') + '</span></div>'; }).join('') : '<span class="xiao6-empty">暂无进行中的任务</span>';
    }
    // quick actions
  ...
  function renderCapabilities() {
    ...
    }).join('') : '<span class="xiao6-empty">暂无能⼒数据</span>';
  }
  function renderSessions() {
    ...
    getJSON('/api/chat/history?limit=50').then(function (d) {
      ...
      }).join('') : '<span class="xiao6-empty">暂无对话记录</span>';
    });
  }
  ...
  function renderResults() {
    ...
    if (!resultLog.length) { list.innerHTML = '<span class="xiao6-empty">暂无结果</span>'; return; }
  ...
  function renderAgent() {
    ...
    if (!agentLog.length) { list.innerHTML = '<span class="xiao6-empty">暂无 Agent 活动</span>'; return; }
  ...
  function renderContext(kind) {
    ...
    var html = '';
    if (kind === 'context') {
      html += ctxCard('运行时', '<div class="xiao6-ctx-state"><span class="xiao6-statedot ' + ((String(snap.agent.state || '').toUpperCase() === 'ERROR') ? 'error' : 'done') + '" id="ctxStateDot"><span class="sd"></span></span><span id="ctxStateText">' + (CORE_TEXT[String(snap.agent.state || 'IDLE').toUpperCase()] || '在线待命') + '</span></div>');
      var w = snap.briefing && snap.briefing.weather;
      html += ctxCard('当前上下文', '<div class="xiao6-ctx-item"><span class="dot"></span><span>' + (w ? (w.condition + ' ' + w.temp + '℃') : '无天气数据') + '</span></div>' + ctxItem('能力 ' + (snap.capabilities || []).length + ' · 记忆 ' + (snap.memories || []).length));
    } else if (kind === 'tasks') {
  ...
  function renderSettings() {
    ...
    body.innerHTML = html;
    body.addEventListener('change', function (e) { var sw = e.target.closest('.xiao6-switch'); if (!sw) return; var k = sw.dataset.key; if (k === 'web') toolModes.web = sw.classList.contains('on'); else if (k === 'think') toolModes.think = sw.classList.contains('on'); else if (k === 'speak') { autoSpeak = sw.classList.contains('on'); try { localStorage.setItem('xiao6_autoSpeak', autoSpeak ? '1' : '0'); } catch (e) {} } });
  }
  ...
  function openFeature(id) {
    ...
    openOverlay(f.name, '能力 · /api/' + id.replace(/-/g, '/').replace('capability/os', 'capability_os'), '<div class="xiao6-loading">读取中…</div>', function () {
      ...
      }).catch(function () { $('overlayBody').innerHTML = '<span class="xiao6-empty">读取失败</span>'; });
    });
  }
  ...
  function loadModelFromConfig(cfg) {
    try { var m = localStorage.getItem('xiao6_model'); if (m) currentModel = m; else if (cfg && cfg.llm && cfg.llm.model) currentModel = cfg.llm.model; } catch (e) {}
    renderModelSelector();
  }
  function renderModelSelector() {
    ...
    MODELS.forEach(function (m) {
      ...
      it.addEventListener('click', function () {
        currentModel = m.id;
        try { localStorage.setItem('xiao6_model', m.id); } catch (e) {}
    ...
    cust.addEventListener('click', function () {
      ...
      if (v && v.trim()) { currentModel = v.trim(); try { localStorage.setItem('xiao6_model', currentModel); } catch (e) {} $('modelLabel').textContent = currentModel; renderModelSelector(); pop.hidden = true; }
    });
  ...
  function loadPermFromConfig(cfg) {
    try { var p = localStorage.getItem('xiao6_policy'); if (p) permMode = p; else if (cfg && (cfg.agent_policy_default || cfg.AGENT_POLICY_DEFAULT)) permMode = (cfg.agent_policy_default || cfg.AGENT_POLICY_DEFAULT); } catch (e) {}
  ...
  function buildPermPop() {
    var btn = $('permBtn'); if (!btn || $('permPop')) return;
    var pop = el('div', 'xiao6-perm-pop'); pop.id = 'permPop'; pop.hidden = true;
    ...
    Array.prototype.forEach.call(pop.querySelectorAll('.xiao6-seg button'), function (b) {
      b.addEventListener('click', function () {
        ...
        try { localStorage.setItem('xiao6_policy', permMode); } catch (e) {}
        renderPerm(); pop.hidden = true;
      });
    });
  }

  /* ---------- 右侧消息跳转条（minimap / scroll-spy） ---------- */
  ...
  function init() {
    // ── 主题：默认浅色（读 /api/config，fallback light）──
    (function () {
      try { var s = localStorage.getItem('xiao6_theme'); document.documentElement.setAttribute('data-theme', s || 'light'); } catch (e) { document.documentElement.setAttribute('data-theme', 'light'); }
    })();
    getJSON('/api/config').then(function (cfg) {
      if (cfg && cfg.theme) { document.documentElement.setAttribute('data-theme', cfg.theme); try { localStorage.setItem('xiao6_theme', cfg.theme); } catch (e) {} }
      loadModelFromConfig(cfg);
      loadPermFromConfig(cfg);
    });

    // nav
    qsa('.xiao6-nav-btn').forEach(function (b) { b.addEventListener('click', function () { switchView(b.dataset.nav); }); });
    // conversation tabs
    ...
    qsa('.xiao6-tool').forEach(function (b) {
      b.addEventListener('click', function () {
        ...
        else if (t === 'speak') { autoSpeak = !autoSpeak; try { localStorage.setItem('xiao6_autoSpeak', autoSpeak ? '1' : '0'); } catch (e) {} }
        else if (t === 'voice') { startVoice(); return; }
        b.classList.toggle('is-on', (t === 'speak' ? autoSpeak : toolModes[t]));
      });
    });
    var speakBtn = qs('.xiao6-tool[data-tool="speak"]'); if (speakBtn) speakBtn.classList.toggle('is-on', autoSpeak);
    // command form
    ...
    // context close
    $('ctxClose').addEventListener('click', function () { $('xiao6Context').parentElement.classList.add('context-collapsed'); });
    ...
    document.addEventListener('keydown', function (e) {
      ...
      else if (e.key === 'Escape') { closeOverlay(); closePalette(); $('xiao6Context').parentElement.classList.remove('context-collapsed'); }
    });
